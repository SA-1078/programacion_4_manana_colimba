...
class PantallaDialogs extends StatelessWidget {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(esError
            ? 'Error: no se pudo conectar al dispositivo móvil'
            : 'Boton nuevo'),
  ...
  Future<void> _mostrarConfirmacion(BuildContext context) async {
    final confirmar = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Eliminar dispositivo móvil'),
        content: const Text(
          '¿Estás seguro de que deseas eliminar iPhone 15 Pro Max 256GB?\n'
    ...
    if (confirmar == true) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Equipo Móvil eliminado correctamente')),
      );
    }
  }
  ...
  Future<void> _mostrarFormulario(BuildContext context) async {
    ...
    await showDialog<void>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Agregar dispositivo móvil'),
        content: Form(
          key: formKey,
          child: Column(
    ...
    if (ctrlNombre.text.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Equipo Móvil "${ctrlNombre.text}" agregado')),
      );
    }
  }
      ...
      body: ListView(
        ...
        children: [
          ...
          OutlinedButton.icon(
            ...
            label: const Text('Eliminar dispositivo móvil (Confirmación)'),
          ),
          const SizedBox(height: 8),
          ...
          FilledButton.tonal(
            ...
            child: const Text('Agregar dispositivo móvil (Formulario)'),
          ),
        ],
      ),
    );
