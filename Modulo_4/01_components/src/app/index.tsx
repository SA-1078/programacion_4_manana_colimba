...
// ═══════════════════════════════════════════════════════════════════
// COMPONENTES COMPARTIDOS (reutilizados desde Paso 2 en adelante)
// ═══════════════════════════════════════════════════════════════════
...
function TarjetaServidor({ nombre, ip, sistema, puerto }: TarjetaServidorProps) {
  return (
    <View style={estilos1.tarjeta}>
      <Text style={estilos1.nombreServidor}>{nombre}</Text>
      <Text style={estilos1.datoDireccion}>
        {ip}:{puerto}
      </Text>
      <Text style={estilos1.datoSistema}>{sistema}</Text>
    </View>
  );
}
...
function Paso1() {
  return (
    <View style={estilos1.fondo}>
      <Text style={estilos1.encabezado}>Inventario de servidores</Text>
      <TarjetaServidor nombre="web-01" ip="10.0.2.10" sistema="Ubuntu 24.04 LTS" puerto={22} />
      <TarjetaServidor
        nombre="db-primary"
        ip="10.0.2.20"
        sistema="Debian 12"
  ...
  return (
    <View style={estilos3.fila}>
      ...
      <View style={{ alignItems: 'flex-end', gap: 4 }}>
        {latencia !== undefined && <Text style={estilos3.latencia}>{latencia} ms</Text>}
...
function BarraUso({ porcentaje, etiqueta }: { porcentaje: number; etiqueta: string }) {
  const color = porcentaje > 85 ? '#c62828' : porcentaje > 60 ? '#f57f17' : '#2e7d32';
    ...
    <SafeAreaView style={estilos5.safeArea}>
      ...
      <ModalConfirm
        ...
        onConfirmar={() => {
          ...
          setAccionEjecutada(
            'Reinicio enviado a web-01 a las ' + new Date().toLocaleTimeString(),
          );
        }}
      ...
      <ScrollView contentContainerStyle={estilos6.scrollContenido} showsVerticalScrollIndicator={false}>
        ...
        <Card titulo="Acciones">
          <View style={estilos6.gridAcciones}>
            <BotonAccion etiqueta="Reiniciar" color="#1565c0" onPress={() => pedirConfirmacion('Reinicio')} />
            <BotonAccion etiqueta="Apagar" color="#b71c1c" onPress={() => pedirConfirmacion('Apagado')} />
            <BotonAccion etiqueta="SSH" color="#2e7d32" onPress={() => pedirConfirmacion('Conexión SSH')} />
            <BotonAccion
              etiqueta="Ver logs"
              color="#6a1b9a"
...
const estilosCard = StyleSheet.create({
  ...
  valor: { fontSize: 13, color: '#1a1a1a', fontFamily: 'monospace' },
});
...
const estilosModal = StyleSheet.create({
  ...
  btnConfirmar: { backgroundColor: '#b71c1c' },
...
const estilos1 = StyleSheet.create({
  ...
  datoDireccion: { fontSize: 13, color: '#1565c0', fontFamily: 'monospace' },
  datoSistema: { fontSize: 12, color: '#757575' },
});
...
const estilos4 = StyleSheet.create({
  ...
  nodoIp: { fontSize: 12, color: '#4fc3f7', fontFamily: 'monospace' },
  nodoOs: { fontSize: 11, color: '#546e7a', marginBottom: 2 },
  barraFila: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  barraEtiqueta: { fontSize: 11, color: '#78909c', width: 28 },
  barraFondo: { flex: 1, height: 4, backgroundColor: '#1c3548', borderRadius: 2, overflow: 'hidden' },
  barraRelleno: { height: '100%', borderRadius: 2 },
  barraPct: { fontSize: 11, fontWeight: '600', width: 34, textAlign: 'right' },
});
...
const estilos5 = StyleSheet.create({
  ...
  subtitulo: { fontSize: 14, color: '#546e7a', marginTop: 4, marginBottom: 32, fontFamily: 'monospace' },
  botonAccion: { backgroundColor: '#b71c1c', borderRadius: 10, paddingVertical: 14, alignItems: 'center' },
  textoAccion: { color: '#fff', fontWeight: '700', fontSize: 15 },
  resultado: { marginTop: 24, backgroundColor: '#e8f5e9', borderRadius: 8, padding: 14 },
  textoResultado: { color: '#2e7d32', fontSize: 13, fontFamily: 'monospace' },
});
...
const estilos6 = StyleSheet.create({
  ...
  cabIp: { fontSize: 12, color: '#78909c', fontFamily: 'monospace', marginTop: 2 },
  ...
  entradaLog: { fontSize: 11, color: '#2e7d32', fontFamily: 'monospace', lineHeight: 18 },
});
